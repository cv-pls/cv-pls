...
(function() {
  ...
  MutationObserverWrapper.prototype.observerCallback = function(mutations) {
    ...
    var i, j, k, l, m, n, y, z,
        newNode, oldNode, nodeArr, filterResult,
        nodes = {
    ...
    if (this.callbacks.nodereplaced.length) {
      ...
      for (i = 0, l = mutations.length; i < l; i++) {
        for (j = 0, m = mutations[i].addedNodes.length; j < m; j++) {
          ...
          } else if (filterResult = applyFilter(this.callbacks.filteradded, newNode)) {
            if (typeof filterResult === 'object') {
              for (y = 0, z = filterResult.length; y < z; y++) {
                nodes.added.push(filterResult[y]);
              }
            } else {
        ...
        for (j = 0, m = mutations[i].removedNodes.length; j < m; j++) {
          ...
          } else if (filterResult = applyFilter(this.callbacks.filterremoved, oldNode)) {
            if (typeof filterResult === 'object') {
              for (y = 0, z = filterResult.length; y < z; y++) {
                nodes.removed.push(filterResult[y]);
              }
            } else {
      ...
      for (i = 0, l = mutations.length; i < l; i++) {
        for (j = 0, m = mutations[i].addedNodes.length; j < m; j++) {
          if (filterResult = applyFilter(this.callbacks.filteradded, mutations[i].addedNodes[j])) {
            if (typeof filterResult === 'object') {
              for (y = 0, z = filterResult.length; y < z; y++) {
                nodes.added.push(filterResult[y]);
              }
        ...
        for (j = 0, m = mutations[i].removedNodes.length; j < m; j++) {
          if (filterResult = applyFilter(this.callbacks.filterremoved, mutations[i].removedNodes[j])) {
            if (typeof filterResult === 'object') {
              for (y = 0, z = filterResult.length; y < z; y++) {
                nodes.removed.push(filterResult[y]);
              }
  ...
  MutationEventWrapper.prototype.processCache = function() {
    var i, j, l, m, y, z,
        oldNode, newNode, nodeArr, filterResult;
    if (this.callbacks.nodereplaced.length) {
      ...
      for (i = 0, l = this.nodeCache.added.length; i < l; i++) {
        ...
        if (oldNode) {
          ...
          if (!filterResult) {
            ...
            this.nodeCache.added.remove(newNode);
            for (y = 0, z = filterResult.length; y < z; y++) {
              this.nodeCache.added.push(filterResult[y]);
            }
          }
        }
      }
      for (i = 0, l = this.nodeCache.removed.length; i < l; i++) {
        filterResult = applyFilter(this.callbacks.filterremoved, this.nodeCache.removed.item(i));
        if (!filterResult) {
          this.nodeCache.removed.remove(this.nodeCache.removed.item(i));
        } else if (typeof filterResult === 'object') {
          this.nodeCache.removed.remove(this.nodeCache.removed.item(i));
          for (y = 0, z = filterResult.length; y < z; y++) {
            this.nodeCache.removed.push(filterResult[y]);
          }
        }
      }
    } else {
      for (i = 0, l = this.nodeCache.added.length; i < l; i++) {
        filterResult = applyFilter(this.callbacks.filteradded, this.nodeCache.added.item(i));
        if (!filterResult) {
